...
def extract_intermediate(filename):
...
def plot_flux_profiles(input_paths, output_dir, tolerance, r_in, theta0, kappaff, kappaK, opt_regime):
    ...
    for idx, input_path in enumerate(input_paths):
       ...
       # max_F = F_all.max() if len(F_all) > 0 else 0
        ...
        chi = extract_intermediate(os.path.basename(input_path))
        ...
        # 第一条线（α ≈ 0）用于图例
        ax.plot(b_0[sorted_idx_0], F_0[sorted_idx_0], label=fr"$\chi = {chi:.1f}/M$", color=color, linestyle='-')
...
if __name__ == "__main__":
    ...
    kappaff = 0.5
    kappaK = 0.3 
    opt_regime = "intermediate" #thin, thick or intermediate 
