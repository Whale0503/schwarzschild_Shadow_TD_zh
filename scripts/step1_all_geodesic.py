...
def compute_by_phi_step(b, r0, dphi, r_max, phi_max, M):
    ...
    while phi <= phi_max:
        ...
        mu = y[0]
        r_current = 1 / mu
        if r_current <= 2 * M or (r_prev < r_max and r_current >= r_max): #掉入黑洞或者从越过r_max, 默认视界在2M处
            break
        phi_vals.append(phi)
        mu_vals.append(mu)
        r_prev = r_current
...
def dphi_dr(r, b, M, b_threshold=1.0):
    numer = b / r**2
    denom_sq = 1 - (1 - 2*M / r) * b**2 / r**2  #默认度规:1/g_{rr}=-g_{tt} = B = 1 - 2M / r
...
if __name__ == "__main__":
    ...
    ymax = config["ymax"]
    #M = 1.0                      # 黑洞质量
    #r0 = 100000.0                # 初始距离（远距离观者位置）
    phi_max = 6 * np.pi        # 最大积分角度
    #step_b = 0.005             # 统一的b步长
    #dphi = 0.005               # 固定角度步长（可根据需要调整）
    #r_max = 50.0  # 定义逃逸距离的阈值 吸积盘的最远距离
    #b_max= 25.0
    #xmax = 25.0
    #ymax = 25.0
    ...
    plot_xlim = (-xmax, xmax)      # 图像x轴范围
    plot_ylim = (-ymax, ymax)      # 图像y轴范围
...
# 定义表头
    ...
    #    np.savetxt(
      #     save_path_data,
       #    np.array(all_data_sorted, dtype=object),
        #   delimiter="\t",
         #  header=header,
         #  comments="",
         #  fmt=["%.3f", "%.9f", "%.3f", "%.9f"]
